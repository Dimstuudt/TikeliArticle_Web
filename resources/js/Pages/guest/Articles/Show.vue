      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="bg-white rounded-lg shadow-md border p-6 space-y-6">
            ...
            <!-- Konten -->
            <div
              class="max-w-none text-gray-900 prose
...
<div class="bg-white rounded-xl shadow p-5 space-y-6">
  <h3 class="text-lg font-bold text-purple-700 border-b border-gray-200 pb-2">
    Rekomendasi Artikel 📰
  </h3>
  ...
  <div
    ...
    :key="rec.id"
    class="flex items-center gap-5 rounded-lg hover:bg-gray-50 transition cursor-pointer p-4"
  >
    <!-- Thumbnail -->
    <a :href="`/articles/${rec.id}`" class="flex-shrink-0">
      <img
        v-if="rec.cover"
        :src="rec.cover"
        alt="Cover"
      ...
      <div
        v-else
        class="w-36 h-24 bg-gray-200 rounded-lg flex items-center justify-center text-gray-400 text-sm"
      >
        No Img
      </div>
    </a>
    ...
    <div class="flex-1 min-w-0">
      ...
      <div class="flex items-center gap-2 mt-1">
        ...
        <span
          class="bg-gradient-to-r from-sky-500 to-cyan-400 text-white text-[11px] font-semibold px-2.5 py-0.5 rounded-full shadow-sm"
        >
          {{ rec.category ?? 'Umum' }}
        </span>
      </div>
